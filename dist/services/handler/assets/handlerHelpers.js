...
exports.getClientIP = exports.bodyParser = exports.cookieParser = exports.buildQuery = void 0;
    ...
    if (request.body.includes('Content-Disposition: form-data; name=')) {
        ...
        while (processingIndex < lines.length - 2) {
            ...
            if (disposition.length === 1) {
                ...
                if (lastKey) {
                    body[lastKey].value += value.length === 0 ? '\n' : `\n${value}`;
                }
                processed++;
            }
            else {
                ...
                const assign = { type: item.type, value: item.value.trim(), name: undefined };
                if (item.filename) {
...
function getClientIP(req, trusted_proxies) {
    const local_ips = helpers_1.default.getLocalIPs();
    ...
    const realIP = (() => {
        if (req.headers['X-Real-IP']) {
            ...
            if (helpers_1.default.validate.ipAddress(real)) {
                return real === '::1' ? local_ips[0] : real;
            }
            else {
                helpers_1.default.printConsole(`The value of the 'X-Real-IP' header is invalid. Expected a valid IP address but got ${real}`);
            }
        }
        return null;
    })();
    ...
    if (Array.isArray(trusted_proxies) && trusted_proxies.includes(remoteAddress)) {
        ...
        if (typeof forwardedFor === 'string' && helpers_1.default.is.validString(forwardedFor)) {
