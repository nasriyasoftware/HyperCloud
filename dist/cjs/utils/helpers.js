...
class Helpers {
    ...
    calculateHash(filePath) {
        return new Promise((resolve, reject) => {
            const hash = crypto_1.default.createHash('sha256'); // You can use other hash algorithms like 'md5', 'sha1', etc.
    ...
    */
    ...
    validate = {
        ...
        currency: (currency) => {
            if (typeof currency === 'string') {
                ...
                return currencies_json_1.default.includes(currency);
            }
            else {
                return false;
            }
        ...
        */
        ...
        projectPath: (projectPath) => {
            ...
            if (validity.valid) {
                const dirs = fs_1.default.readdirSync(projectPath);
    ...
    checkPathAccessibility(path) {
        ...
        checks.exist = fs_1.default.existsSync(path);
        if (!checks.exist) {
            return { valid: false, errors: checks };
        }
        try {
            fs_1.default.accessSync(path, fs_1.constants.R_OK | fs_1.constants.W_OK);
