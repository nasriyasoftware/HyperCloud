            ...
            try {
                ...
                for (let i = 0; i < reqPath.length; i++) {
                    ...
                    if (isLast) {
                        ...
                        const folder = path_1.default.resolve(path_1.default.join(this.#_root, ...copy));
                        // Check folder path validity                       
                        const validity = helpers_1.default.checkPathAccessibility(folder);
                        if (validity.valid !== true) {
                            return next();
                        }
                        // Check if the path is an actual directory
                        const folderStats = fs_1.default.statSync(folder);
                        if (!folderStats.isDirectory()) {
                            return next();
                        }
                        const filename = pathSegment;
                        // Read the content of the folder
                        const content = fs_1.default.readdirSync(folder, { withFileTypes: true });
                        ...
                        const eTagsPath = path_1.default.join(folder, 'eTags.json');
                        const eTagValidity = helpers_1.default.checkPathAccessibility(eTagsPath);
                        if (eTagValidity.valid) {
                            const eTags = JSON.parse(fs_1.default.readFileSync(eTagsPath, { encoding: 'utf-8' }));
                            if (helpers_1.default.is.realObject(eTags)) {
