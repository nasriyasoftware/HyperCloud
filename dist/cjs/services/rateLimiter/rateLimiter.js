...
const RateLimitingRecord_1 = __importDefault(require("./RateLimitingRecord"));
...
class RateLimitingManager {
    ...
    defineRule(rule) {
        ...
        if ('cooldown' in rule) {
            if (!helpers_1.default.is.integer(rule.cooldown)) {
        ...
        if ('rate' in rule) {
            if (helpers_1.default.is.realObject(rule.rate)) {
                const rate = rule.rate;
                if ('windowMs' in rate) {
                    if (!helpers_1.default.is.integer(rate.windowMs)) {
                ...
                if ('maxRequests' in rate) {
                    if (!helpers_1.default.is.integer(rate.maxRequests)) {
     ...
     */
    authorize(options) {
        if (helpers_1.default.is.undefined(options)) {
            throw new SyntaxError(`The 'authorize()' method expects an 'options' argument`);
        }
        if (!helpers_1.default.is.realObject(options)) {
        ...
        const validRules = options.rules.filter(rule => helpers_1.default.is.realObject(rule) && 'name' in rule).map(rule => {
        ...
        for (const rule of validRules) {
            ...
            if (ruleDetails) {
                ...
                if (!record) {
                    record = new RateLimitingRecord_1.default(ruleDetails, recordKey);
...
exports.default = RateLimitingManager;
