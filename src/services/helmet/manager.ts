import { HelmetConfigOptions, HyperCloudRequestHandler } from "../../docs/docs";
...
class HelmetManager {
    ...
    #_updateHandler() {
        ...
        const handler: HyperCloudRequestHandler = (req, res, next) => {
            helpers.printConsole(this.#_values);
            try {
                // #1: Content-Security-Policy
                ...
                if (this.#_values.crossOriginOpenerPolicy) { res.setHeader("Cross-Origin-Opener-Policy", this.#_values.crossOriginOpenerPolicy) }
                // #4: Cross-Origin-Resource-Policy
                if (this.#_values.crossOriginResourcePolicy) { res.setHeader("Cross-Origin-Resource-Policy", this.#_values.crossOriginResourcePolicy) }
                // #5: Origin-Agent-Cluster
                if (this.#_values.crossOriginResourcePolicy) { res.setHeader("Origin-Agent-Cluster", this.#_values.crossOriginResourcePolicy) }
                // #6: Referrer-Policy
                if (this.#_values.referrerPolicy) { res.setHeader("Referrer-Policy", this.#_values.referrerPolicy) }
                // #7: Strict-Transport-Security
                if (this.#_values.strictTransportSecurity) { res.setHeader("Strict-Transport-Security", this.#_values.strictTransportSecurity) }
                // #8: X-Content-Type-Options
                if (this.#_values.xContentTypeOptions) { res.setHeader("X-Content-Type-Options", this.#_values.xContentTypeOptions) }
                // #9: X-DNS-Prefetch-Control
                if (this.#_values.xDnsPrefetchControl) { res.setHeader("X-DNS-Prefetch-Control", this.#_values.xDnsPrefetchControl) }
                // #10: X-Download-Options
                if (this.#_values.xDownloadOptions) { res.setHeader("X-Download-Options", this.#_values.xDownloadOptions) }
                // #11: X-Frame-Options
                if (this.#_values.xFrameOptions) { res.setHeader("X-Frame-Options", this.#_values.xFrameOptions) }
                // #12: X-Permitted-Cross-Domain-Policies
                if (this.#_values.xPermittedCrossDomainPolicies) { res.setHeader("X-Permitted-Cross-Domain-Policies", this.#_values.xPermittedCrossDomainPolicies) }
                // #13: X-Powered-By
                if (this.#_values.xPoweredBy) { res.removeHeader("X-Powered-By") }
                // #14: X-XSS-Protection
                if (this.#_values.xXssProtection) { res.setHeader("X-XSS-Protection", '0') }
            } catch (error) {
                console.error(error);
            } finally {
                next();
