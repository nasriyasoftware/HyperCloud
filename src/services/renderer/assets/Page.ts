...
export class Page {
    ...
    readonly metaTags = {
        add: {
            ...
            customTag: (attributes: Record<string, string>) => {
                if (!helpers.is.realObject(attributes)) { throw this.#_helpers.createError(`Unable to create a custom meta tag. The method expects an "attributes" object, but instead got ${typeof attributes}`) }
    ...
    readonly title = {
        ...
        multilingual: {
            set: (titles: Record<string, string>) => {
                if (!helpers.is.realObject(titles)) { throw this.#_helpers.createError(`Invalid titles have been passed to the ${this.#_name} page's "title.multilingual.set()". Expected a real object bust instead got ${typeof titles}`) }
    ...
    readonly description = {
        ...
        multilingual: {
            set: (descriptions: Record<string, string>) => {
                if (!helpers.is.realObject(descriptions)) { throw this.#_helpers.createError(`Invalid descriptions have been passed to the ${this.#_name} page's "description.multilingual.set()". Expected a real object bust instead got ${typeof descriptions}`) }
    ...
    readonly locals = {
        ...
        add: (locals: Record<string, any>, lang?: string) => {
            if (!helpers.is.realObject(locals)) { throw this.#_helpers.createError(`An invalid locale has been passed to the ${this.#_name} page's "locals.add()". Expected a real object bust instead got ${typeof locals}`) }
            if (lang === undefined) { lang = 'default' }
        ...
        multilingual: {
            set: (locals: Record<string, Record<string, any>>) => {
                if (!helpers.is.realObject(locals)) { throw this.#_helpers.createError(`An invalid locale has been passed to the ${this.#_name} page's "locals.multilingual.set()" locale. Expected a real object bust instead got ${typeof locals}`) }
